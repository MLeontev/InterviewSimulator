...
internal static class GigaChatPromptFactory
{
    ...
    public static string BuildCodingSystemPrompt() =>
        ...
        - feedback должен быть конкретным и понятным
        - feedback должен кратко объяснять, что получилось хорошо и что стоит улучшить
        - обязательно указывай и сравнивай асимптотическую сложность (О-большое, например O(n), O(n^2)) решения кандидата и эталонного решения
        - если решение неэффективно по времени, объясни, как снижение сложности поможет избежать тайм-аутов
        - СТРОГОЕ ПРАВИЛО: не снижай балл и не делай замечаний за использование вывода в консоль (например, print) вместо возврата значения (return)\
        - НЕ требуй «защитного программирования» (try-except, проверка типов, обработка некорректного ввода), если это не указано в задаче. В алгоритмических секциях мы предполагаем, что входные данные всегда соответствуют формату.
        - Если решение алгоритмически оптимально (соответствует сложности в эталоне), следует формату stdin/stdout и проходит все тесты — СТАВЬ 10/10. Не ищи мелкие недостатки в стиле кода, если логика безупречна.
        ...
        2) Затем качество кода (читаемость, структура, алгоритмическая сложность), но без противоречий автопроверке.
    ...
    public static string BuildCodingUserPrompt(CodingEvaluationRequest request) =>
        $"""
         ...
         - решение должно выводить результат в standard output (через print или аналог)
         - если кандидат отправил только функцию без полноценного ввода/вывода, это несоответствие формату задачи
         - СТРОГОЕ ПРАВИЛО: Не делай замечаний за то, что результат выводится в stdout вместо возврата через return
