...
internal class CodeExecutor(IExecutorLanguageProvider languageProvider, IDockerRunner dockerRunner) : ICodeExecutor
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            runCommand = $"bash -c \"{timedRunCommand} < /code/{InputFile} 2> >(head -c 10485760 > /code/{StdErrFile}) | head -c 10485760 > /code/{StdOutFile}; exit_code=\\${{PIPESTATUS[0]}}; wait; exit \\$exit_code\"";
        }
        else
        {
            var ulimitPrefix = "ulimit -f 20480 && ";
            runCommand = $"{ulimitPrefix}{timedRunCommand} < /code/{InputFile} > /code/{StdOutFile} 2> /code/{StdErrFile}";
        }
