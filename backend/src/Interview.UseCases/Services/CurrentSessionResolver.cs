...
using InterviewSessionEntity = Interview.Domain.Entities.InterviewSession;

namespace Interview.UseCases.Services;

internal interface ICurrentSessionResolver
{
    Task<InterviewSessionEntity?> GetCurrentSessionAsync(Guid candidateId, CancellationToken ct);
...
{
    public async Task<InterviewSessionEntity?> GetCurrentSessionAsync(Guid candidateId, CancellationToken ct) => 
